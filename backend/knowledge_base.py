...
def _entry_text(fields: dict) -> str:
    return " ".join(
        str(fields.get(k, "")) for k in (
            "知识描述", "具体方法", "适用条件", "判断逻辑",
            "knowledge_desc", "knowledge_ref", "rule_ref", "output",
        )
    )
    ...
    *,
    ...
    try:
        ...
        for entry in (ir or {}).get("entries") or []:
            ...
            if not text.strip():
                continue
            confidence = str(fields.get("置信度", "") or fields.get("confidence", ""))
            contributed = str(fields.get("贡献专家", "")) or fields.get("source_label", "") or by
            try:
