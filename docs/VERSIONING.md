...
| 字段 | 来源 | 示例 |
|------|------|------|
| `app_version` | 项目根目录 `VERSION` 文件（语义化版本） | `3.0.0` |
| `image_version` / `build_id` | 构建时注入，格式 `{app_version}-arm64-{yyyyMMdd-HHmm}` | `3.0.0-arm64-{时间}` |
...
```json
{
  ...
  "app_version": "3.0.0",
  "image_version": "3.0.0-arm64-{时间}",
  "build_id": "3.0.0-arm64-{时间}",
...
- **推荐（可追踪）**：`tacit-knowledge-externalization:3.0.0-arm64-{时间}`
- **次要别名**：`tacit-knowledge-externalization:3.0.0-arm64`（同次构建会覆盖，勿用于多版本并存）
